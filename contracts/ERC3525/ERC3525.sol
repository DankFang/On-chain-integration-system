...
contract ERC3525 is Context, IERC3525Metadata, IERC721Enumerable {
    ...
    modifier onlyUsePtoken {
        require(msg.sender == usePtokenAddr);
        _;
    }

    IERC3525MetadataDescriptor public metadataDescriptor;

    constructor(string memory name_, string memory symbol_, uint8 decimals_, usePtokenAddr_) {
        _tokenIdGenerator = 1;
        _name = name_;
        _symbol = symbol_;
        _decimals = decimals_;
        usePtokenAddr = usePtokenAddr_;
    }
    ...
    function transferFrom(
        ...
        address to_,
        uint256 value_
    ) public payable onlyUsePtoken virtual override returns (uint256 newTokenId) {
    ...
    function transferFrom(
        ...
        uint256 toTokenId_,
        uint256 value_
    ) public payable onlyUsePtoken virtual override {
        _spendAllowance(_msgSender(), fromTokenId_, value_);
